...
export function AdvisorProposal() {
	...
	useEffect(() => {
		async function fetchRecommendations() {
			await query
				?.queryID<Recommendation[]>("recommendations")
				.then((data) => {
	...
	useEffect(() => {
		async function fetchAdvisors() {
			if (recommendations) {
				...
				await query
					?.queryInfo<AdvisorData>(
						"advisors",
						recommendations?.map((r) => r.advisor_id)
					)
					.then((data) => {
						setAdvisors(data)
	...
	const handleProposal = async (advisor: AdvisorData) => {
		await query
			?.postActionWithBody("proposals", { advisor_id: advisor.user_id })
			.then(() => {
				alert?.openAlert("success", "advisor_proposal")
				user?.getUser()
			})
			.catch((err) => {
	...
	if (!user?.user?.student?.is_advisors_recommended) {
		return <Alert severity="info">Wait for advisor recommendation!</Alert>
	}

	if (user?.user?.student?.is_approved) {
		return <Alert severity="info">You are approved!</Alert>
	}
	return (
		<div>
				...
				<UserTable title={"Previously Uploaded Advisors"}>
					{advisors?.map((advisor) => (
						<TableRow key={"table_row_" + advisor.name_}>
							<TableCell>
								<IconButton onClick={() => handleProposal(advisor)}>
